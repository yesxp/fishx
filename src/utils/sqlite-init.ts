// SQLite 初始化：建表 + 种子数据
// 首次启动执行；schema 版本升级时手动迁移

import { exec, query, persist } from './sqlite'
...
const CREATE_TABLES = [
  ...
  `CREATE TABLE IF NOT EXISTS catches (
    ...
    species_confidence REAL NOT NULL DEFAULT 0,
    photo_urls TEXT NOT NULL DEFAULT '[]',
    primary_photo_index INTEGER NOT NULL DEFAULT 0,
    weight_estimated_g INTEGER NOT NULL DEFAULT 0,
    weight_g INTEGER,
    weight_confirmed INTEGER NOT NULL DEFAULT 0,
    spot_geo_lat REAL NOT NULL,
    spot_geo_lng REAL NOT NULL,
  ...
  // 鱼种-用户战绩视图（运行时统计）
  `CREATE TABLE IF NOT EXISTS species_user_stats (
    species_id TEXT PRIMARY KEY,
    user_id TEXT NOT NULL,
    catch_count INTEGER NOT NULL DEFAULT 0,
    best_weight_g INTEGER,
...
const INDEXES = [
  ...
  `CREATE INDEX IF NOT EXISTS idx_species_user_stats_user ON species_user_stats(user_id)`,
]
...
const SPECIES_SEED = [
  {
    ...
    zh_name: '鲫鱼',
    emoji: '🐟',
    common_names: JSON.stringify(['鲫瓜子', '河鲫', '月鲫']),
    scientific_name: 'Carassius auratus',
    family: '鲤科',
    difficulty: '易',
    water_layer: '底层',
    feeding: '杂食',
    season: JSON.stringify(['春', '夏', '秋', '冬']),
    ...
    copywriting_seeds: JSON.stringify({
    ...
    related_species: JSON.stringify(['sp-002', 'sp-003', 'sp-008']),
  },
  {
    id: 'sp-002',
    zh_name: '鲤鱼',
    emoji: '🐠',
    common_names: JSON.stringify(['鲤子', '拐子', '红尾']),
    scientific_name: 'Cyprinus carpio',
    family: '鲤科',
    difficulty: '中',
    water_layer: '底层',
    feeding: '杂食',
    season: JSON.stringify(['春', '夏', '秋']),
    ...
    related_species: JSON.stringify(['sp-001', 'sp-003', 'sp-008']),
  },
  {
    id: 'sp-003',
    zh_name: '草鱼',
    emoji: '🐡',
    common_names: JSON.stringify(['草鲩', '白鲩', '厚子']),
    scientific_name: 'Ctenopharyngodon idella',
    family: '鲤科',
    difficulty: '中',
    water_layer: '中下',
    feeding: '草食',
    season: JSON.stringify(['夏', '秋']),
    ...
    related_species: JSON.stringify(['sp-001', 'sp-002', 'sp-008']),
  },
  {
    id: 'sp-004',
    zh_name: '鲈鱼',
    emoji: '🎏',
    common_names: JSON.stringify(['鲈子', '花鲈', '七星鲈']),
    scientific_name: 'Lateolabrax japonicus',
    family: '鮨科',
    difficulty: '中',
    water_layer: '中上',
    feeding: '肉食',
    season: JSON.stringify(['春', '秋', '冬']),
    ...
    related_species: JSON.stringify(['sp-005', 'sp-007']),
  },
  {
    id: 'sp-005',
    zh_name: '翘嘴',
    emoji: '🐟',
    common_names: JSON.stringify(['翘嘴鲌', '白鱼', '翘壳']),
    ...
    related_species: JSON.stringify(['sp-002', 'sp-003', 'sp-004']),
  },
  {
    id: 'sp-006',
    zh_name: '罗非',
    emoji: '🐠',
    common_names: JSON.stringify(['非洲鲫', '福寿鱼', '越南鱼']),
    ...
    related_species: JSON.stringify(['sp-001', 'sp-007']),
  },
  {
    id: 'sp-007',
    zh_name: '鲶鱼',
    emoji: '🐡',
    common_names: JSON.stringify(['鲶拐', '土鲶', '胡子鱼']),
    ...
    related_species: JSON.stringify(['sp-004', 'sp-006']),
  },
  {
    id: 'sp-008',
    zh_name: '鳊鱼',
    emoji: '🎏',
    common_names: JSON.stringify(['长春鳊', '鳊花', '武昌鱼']),
    ...
    related_species: JSON.stringify(['sp-001', 'sp-002', 'sp-003']),
  },
]
...
export function initSchema(): void {
  // 1. 建表
  CREATE_TABLES.forEach(sql => exec(sql))

  // 2. 建索引
  INDEXES.forEach(sql => exec(sql))

  // 3. 检查是否已种子
  const count = query<{ c: number }>('SELECT COUNT(*) as c FROM fish_species')
  if (count[0]?.c > 0) {
    console.log('[SQLite] 种子已存在，跳过')
    persist()
    return
  }

  // 4. 插种子
  console.log('[SQLite] 插入 8 鱼种种子...')
  SPECIES_SEED.forEach(s => {
    exec(
      `INSERT INTO fish_species (
        id, zh_name, emoji, common_names, scientific_name, family,
        difficulty, water_layer, feeding, season, weight_range, distribution,
        subtitle, description, one_fish_one_name, fun_facts, taste, tactics,
        copywriting_seeds, related_species
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        s.id, s.zh_name, s.emoji, s.common_names, s.scientific_name, s.family,
        s.difficulty, s.water_layer, s.feeding, s.season, s.weight_range, s.distribution,
        s.subtitle, s.description, s.one_fish_one_name, s.fun_facts, s.taste, s.tactics,
        s.copywriting_seeds, s.related_species,
      ]
    )
  })
